...
LastEditTime: 2020-08-31 15:01:50
FilePath: /newBookClassification/features.py
Desciption:  
'''
import numpy as np
import pandas as pd
...
def label2idx(data):
    ...
    if os.path.exists('./data/label2id.json'):
        ...
        firstName = data['firstClass'].unique()
        secondName = data['secondClass'].unique()
        label = firstName.tolist() + secondName.tolist()
...
def get_tfidf(tfidf, data):
    data_tfidf = pd.DataFrame(
        tfidf.transform(
            data["text"].apply(lambda x: " ".join(x)).tolist()).toarray())
...
def get_embedding_feature(data, embedding_model):
    ...
    w2v_label_embedding = np.array([
        np.mean([
            embedding_model.wv.get_vector(word) for word in jieba.cut(key)
